...
describe('Testando o Middleware /cadastrar', function(done){
	describe('Validation', function(done){
		...
		it('Validando todos os campos nulos', function(done){
			var next = function(){};
			validation.alterar(req, res, next);
			(res['stat']).should.be.exactly(400);
			(res['errs']).should.have.length(4);
			(res['errs'][0]).should.have.property('campo', 'id');
			(res['errs'][0]).should.have.property('mensagem', 'Id inválido.');
			(res['errs'][1]).should.have.property('campo', 'nome');
			(res['errs'][1]).should.have.property('mensagem', 'Digite o nome');
			(res['errs'][2]).should.have.property('campo', 'email');
			(res['errs'][2]).should.have.property('mensagem', 'Digite o email');
			(res['errs'][3]).should.have.property('campo', 'senha');
			(res['errs'][3]).should.have.property('mensagem', 'Digite a senha');
			done();
		});
		describe('NOME', function(done){
			...
			it('Validando o campo nome inferior de 5 caracteres', function(done){
				...
				tests.forEach(function(test, index){
					...
					(res['errs']).should.have.length(4);
					(res['errs'][1]).should.have.property('campo', 'nome');
					(res['errs'][1]).should.have.property('mensagem', 'Nome inferior de 5 caracteres');
					(req.body.nome === expect[index]).should.equal(true);
					before();
				})
				done();
		...
		describe('EMAIL', function(done){
			...
			it('Validando o campo email invalido', function(done){
				...
				tests.forEach(function(test, index){
					...
					(res['errs']).should.have.length(4);
					(res['errs'][2]).should.have.property('campo', 'email');
					(res['errs'][2]).should.have.property('mensagem', 'Email invalido');
					before();
				})
				done();		
			});
			it('Validando o campo email normalizeEmail', function(done){
				...
				tests.forEach(function(test, index){
					...
					(res['stat']).should.be.exactly(400);
					(res['errs']).should.have.length(3);
					(res['errs'][2]).should.have.not.property('campo', 'email');
					(res['errs'][2]).should.have.not.property('mensagem', 'Email invalido');
					(req.body.email === expect[index]).should.equal(true);
					before();
				})
				done();		
		...
		describe('SENHA', function(done){
			it('Validando o campo senha com nulos', function(done){
				...
				tests.forEach(function(test, index){
					...
					(res['stat']).should.be.exactly(400);
					(res['errs']).should.have.length(4);
					(res['errs'][3]).should.be.type('object');
					(res['errs'][3]).should.have.property('campo', 'senha');
					(res['errs'][3]).should.have.property('mensagem', 'Digite a senha');
			...
			it('Validando o campo senha de tamanho invalido. (de 5 á 12 caracteres)', function(done){
				...
				tests.forEach(function(test, index){
					...
					(res['errs'][3]).should.be.type('object');
					(res['errs'][3]).should.have.property('campo', 'senha');
					(res['errs'][3]).should.have.property('mensagem', 'Senha de tamanho invalido. (de 5 á 12 caracteres)');
					before();
				})
				done();
			});
